...
syn region	mqString		start=+"+  skip=+\\\\\|\\"+  end=+"+  contains=mqSpecial
syn region	mqString		start=+'+  skip=+\\\\\|\\'+  end=+'+  contains=mqSpecial
syn region	mqString		start=+`+  skip=+\\\\\|\\`+  end=+`+  contains=mqSpecial

syn keyword	mqElementNode		program library sources link build cflags cppflags custom ex li
syn keyword	mqElementNode		extra_dist nodist_sources rule ldflags project section headers
