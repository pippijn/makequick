...
Make++ defines a few rules itself and has knowledge about automake-defined
rules, so that e.g. rules for compiling C sources are not emitted again. The
rules feature in Make++ is the reason this project was actually started. The
...
Rules are matched on the graph, rather than on the filesystem. When a rule
...
this case, C<$(THIS)> is expanded to C<$(builddir)/msgcat$(EXEEXT)>
...
The introductory example is already a pattern rule. Patterns in Make++ are
...
instead of a subdir. For this use case, the C<%%> pattern may be used.
...
C<@>. Make++ extends this to automake C<silent-rules>.
...
If Make++ is not flexible enough for the use case, external rules may be
...
C<parallel-tests> facilities. These tools are the reason Make++ targets automake
...
lower level than Make++, so it is not advisable to override many of its rules.
