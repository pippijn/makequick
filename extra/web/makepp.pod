...
Make++ is a make-inspired build system that compiles to GNU Autotools.
...
working piece of software, C<Make++> is nowhere, yet. At the moment, there are
only ideas, but not much code. The ideas are also very much in flux, so don't
expect much stability.

=head2 Introduction

Make++ compiles a purely declarative build definition to POSIX-compliant and
...
definition as C<libb.la liba.la> would work. Make++ solves this problem by
...
withheld from advanced users. Make++ supports but discourages the use of
wildcards.

=head3 Autotools integration

Make++ input is translated to one or more C<Makefile.am>s and a single
...
formats. Make++ attempts to unify these formats in a new format based on QML and
GNU Make.

=head3 Flexibility

Make++ is, by design, much less flexible than automake and autoconf. It is also
much more strict. It gives the user some additional tools the author has been
...
a black box to Make++, thus defeating its static analysis features. Make++
...
Make++ does not plan on supporting percentages, because the author does not
